...
public sealed class ErosionParams
{
    ...
    public float AccumRate  = 0.5f;    // flow-accumulation relaxation per step (stability as h shifts)
    public float ChannelThreshold = 80f; // upstream-area A above which a cell counts as a channel
    public float FlowExp    = 4.0f;    // D8 slope^p routing exponent; higher = sharper near-steepest channels

    // std430: slot 0 = res (int bits), slots 1..17 floats, 18..19 pad → 20 scalars = 80 bytes.
    public byte[] Pack()
    {
        var f = new float[20];
        ...
        f[16] = ChannelThreshold; f[17] = FlowExp; // 18-19 pad (0)
