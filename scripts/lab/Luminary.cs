...
///
/// "3 suns without killing the computer" lives in `LuminaryBudget`: the scarce hardware (≤4 Godot
/// directional lights, ≤2 shadow atlases, ≤3 atmosphere-scattered suns) is allocated to the highest-
/// priority luminaries; everyone else renders as a cheap disc + summed ambient tint. Shadow passes are
/// the dominant cost and stay capped REGARDLESS of luminary count — so 3 suns ≈ today's frame.
...
public sealed class Luminary
{
    ...
    public bool CastsShadow = true;           // wants a shadow atlas? (granted only to the top caps.MaxShadowCasters)
...
/// Scarce-resource budget. Data (not constants) so a low-end profile can drop shadow casters to 1 and
/// atmosphere suns to 1 without code edits. Defaults match the spec's table.
public sealed class LuminaryCaps
{
    ...
    public int MaxShadowCasters = 2;    // each = a full CSM pass, the frame's costliest item (1 full + 1 reduced)
...
public sealed class LuminaryAllocation
{
    ...
    public bool[] Shadow;       // granted a shadow atlas?
    ...
    {
        LightSlot = new int[n]; Shadow = new bool[n]; Atmosphere = new bool[n]; Disc = new int[n];
...
public static class LuminaryBudget
{
    ...
    public static LuminaryAllocation Allocate(IReadOnlyList<Luminary> lums, IReadOnlyList<float> weights, LuminaryCaps caps)
    {
        ...
        int lightsUsed = 0, shadowsUsed = 0, atmoUsed = 0, discsUsed = 0;
        foreach (int i in order)
        {
            ...
            // Shadow atlas: only a physical light can cast; top MaxShadowCasters of those that want it.
            if (L.CastsShadow)
            {
                if (a.LightSlot[i] >= 0 && shadowsUsed < caps.MaxShadowCasters) { a.Shadow[i] = true; shadowsUsed++; }
                else { a.Notes.Add($"'{L.Id}' shadowless (shadow cap {caps.MaxShadowCasters} reached or no light slot)"); }
            }
